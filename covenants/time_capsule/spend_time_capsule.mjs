...
const COV_TXID = 'e9bbc95a370406e3a504fa9109b3cab74238ae028faec3345a69583bfbcab946';
const COV_IDX = 0;
const COV_AMT = 15000000n;
const COV_SPK_HEX = 'aa207efbe49c8deab7a4d29b651a1742653219e22a57f01645f0ab88905c085b296387';
const REDEEM_SCRIPT_HEX = fs.readFileSync('./full_time_capsule_script.hex', 'utf8').trim();
const WITNESS = JSON.parse(fs.readFileSync('./time_capsule_witness.json', 'utf8'));
...
async function main() {
  ...
  const LOCKTIME = Number(tipInfo.virtualDaaScore);
  console.log('Using lockTime:', LOCKTIME);
  ...
  function buildSighash(idx) {
    ...
    const preimage = Buffer.concat([
      ...
      h_out,
      u64le(LOCKTIME), Buffer.alloc(20), u64le(0n), Buffer.alloc(32),
      Buffer.from([0x01]),
    ]);
    return kh(preimage);
  }
  ...
  const txObj = {
    ...
    lockTime: LOCKTIME, subnetworkId: '0000000000000000000000000000000000000000', gas: 0, payload: '',
  };
