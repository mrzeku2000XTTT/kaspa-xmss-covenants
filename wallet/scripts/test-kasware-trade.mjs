...
import {
  ...
  signPsktWithKasware, fetchKaswareUtxos, repairSafeJson
} from '../js/kasware.js';
...
assert.equal(captured.options.broadcast, undefined, 'do not pass extra KasWare options that rewrite the PSKT');
...
assert.equal(repairSafeJson('{"a":1,}'), '{"a":1}');
assert.equal(repairSafeJson('{"inputs":[{},]}'), '{"inputs":[{}]}');
const huge = '{"inputs":[' + '{"x":1},'.repeat(3) + ']}';
assert.equal(JSON.parse(repairSafeJson(huge)).inputs.length, 3);
