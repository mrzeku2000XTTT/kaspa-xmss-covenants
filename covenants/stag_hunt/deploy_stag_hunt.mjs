...
const DEPOSIT = 150_000_000n; // 1.5 KAS = the full game pot (D)
const FEE = 5_000_000n;

async function main() {
  const V_ADDR = scripts.covenant_address;
  ...
  const txObj = {
    ...
    outputs: [
      { value: Number(out0val), scriptPublicKey: { script: scripts.spk_hex, version: 0 } },
  ...
  console.log('RESULT_JSON:', JSON.stringify({ txId: r.transactionId, covenantAddress: V_ADDR, depositSompi: DEPOSIT.toString(), explorerUrl: 'https://kaspa.stream/transactions/' + r.transactionId }));
}
