...
const DNA_LEN: usize = 955;
// Chromosome 20: minimum HTLC lock window -- a swap timeout must be set at least this
// far in the future at INIT time, so a counterparty on the other chain always has a
// real, non-instant deadline to work with (the actual real-world atomic-swap pattern
// staggers timeouts across the two chains for safety margin; here we just floor it).
const MIN_SWAP_WINDOW_DAA: u64 = 100;
...
// ── Stage 17 -- "111" (the angel number: new beginnings, alignment) ──
// A phoenix that just rose from absolute dust must rest in the cradle for exactly
// 111 DAA before its first true step out of STEM. A second, shorter mirror of
// Stage 16's three-day tomb: that one gates the RISING itself, this one gates the
// first departure AFTER rising.
const INCUBATION_DAA: u64 = 111;

// ── Stage 18 -- "A SCORPION WHO GENERATES WORLDS" (Chromosome 18, world genesis) ──
// Reproduction (Chromosome 6) spawns more scorpions -- same DNA shape, same species.
// World genesis is different in kind: the scorpion authorizes a brand-new covenant
// genesis output that does NOT have to be a scorpion at all -- any redeem-script
// template, any rule set, any species. What stays permanently verifiable is
// provenance: every world this scorpion has ever seeded is linked into one forward
// hash chain (world_chain_hash), the same memory-chain technique as Chromosome 7,
// applied to children of a different kind. It never resets, not even through death
// and rebirth (Chromosome 14/17) -- the scorpion's memory of what it created is eternal.
...
// ASSEMBLERS (who batch and chain data, scaling with raw bandwidth/CPU) and one thin
// CONSENTER (who only orders/finalizes batch commitments, not individual items). This
// chromosome brings that idea into the organism's own heartbeat: an off-chain assembler
// pre-computes an entire internal chain of ordinary VAULT->VAULT moults (each hop
// obeying the exact same per-hop rules as a normal heartbeat), and ONE on-chain proof
// (the consenter) verifies the whole internal chain and finalizes only the LAST state.
// generation and transition_count advance by batch_size instead of a fixed 1 -- the
// organism's own throughput scales horizontally with how much its assembler prepared,
// while the on-chain footprint stays exactly one transition, same as always.
...
struct Dna {
    ...
    dusted_at_daa: u64,
    // ── Chromosome 17 -- NEWBORN INCUBATION ("111", the angel number) ──
    reborn_at_daa: u64,
    // ── Chromosome 18 -- WORLD GENESIS (a scorpion who generates worlds) ──
    world_chain_hash: [u8; 32],
    worlds_spawned: u32,
    max_worlds: u32,
    // ── Chromosome 19 -- ARMA (assembly/consent separation) ──
    last_batch_size: u32,
    total_batched_transitions: u64,
    // ── Chromosome 20 -- CROSSCHAIN ATOMIC TRANSFER (HTLC: hash + time locked swap) ──
    swap_hash: [u8; 32],
    swap_timeout_daa: u64,
    swap_active: u8,
}
...
fn parse_dna(b: &[u8]) -> Dna {
    ...
    let reborn_at_daa = u64::from_le_bytes(b[854..862].try_into().unwrap());

    let mut world_chain_hash = [0u8; 32];
    world_chain_hash.copy_from_slice(&b[862..894]);
    let worlds_spawned = u32::from_le_bytes(b[894..898].try_into().unwrap());
    let max_worlds = u32::from_le_bytes(b[898..902].try_into().unwrap());

    let last_batch_size = u32::from_le_bytes(b[902..906].try_into().unwrap());
    let total_batched_transitions = u64::from_le_bytes(b[906..914].try_into().unwrap());

    let mut swap_hash = [0u8; 32];
    swap_hash.copy_from_slice(&b[914..946]);
    let swap_timeout_daa = u64::from_le_bytes(b[946..954].try_into().unwrap());
    let swap_active = b[954];
...
// ── Chromosome 19 (ARMA): verifies one internal hop of an assembled batch. ──
// `prev` -> `curr` must be an ordinary VAULT->VAULT heartbeat: same covenant_id, same
// birth certificate, generation/transition_count advance by exactly 1, memory chain
// linked, fee_reserve strictly decreasing and never hitting zero mid-batch. Every OTHER
// byte in the DNA (every chromosome not explicitly named here) must be frozen -- an
// assembled batch cannot smuggle through a mode change, a reproduction, a world seeded,
// or any other kind of event. It is purely the passage of ordinary heartbeats, assembled
// in bulk and consented to all at once.
fn assert_hop_valid(prev: &[u8], curr: &[u8]) {
    assert_eq!(prev.len(), DNA_LEN, "hop: prev DNA must be exactly DNA_LEN bytes");
    assert_eq!(curr.len(), DNA_LEN, "hop: curr DNA must be exactly DNA_LEN bytes");
    assert_eq!(curr[40], MODE_VAULT, "arma batch hop must stay in vault mode");
    assert_eq!(curr[41], MODE_VAULT, "arma batch hop prev_mode must be vault (no mode change mid-batch)");
    assert_eq!(prev[40], MODE_VAULT, "arma batch hop must start from vault mode");
    assert_eq!(&curr[4..36], &prev[4..36], "covenant_id must not change mid-batch");

    let prev_gen = u32::from_le_bytes(prev[36..40].try_into().unwrap());
    let curr_gen = u32::from_le_bytes(curr[36..40].try_into().unwrap());
    assert_eq!(curr_gen, prev_gen + 1, "generation must advance by exactly 1 per hop");

    let prev_tc = u32::from_le_bytes(prev[525..529].try_into().unwrap());
    let curr_tc = u32::from_le_bytes(curr[525..529].try_into().unwrap());
    assert_eq!(curr_tc, prev_tc + 1, "transition_count must advance by exactly 1 per hop");

    assert_eq!(&curr[529..569], &prev[529..569], "creation_daa/creation_txid immutable mid-batch");

    let prev_fee = u64::from_le_bytes(prev[685..693].try_into().unwrap());
    let curr_fee = u64::from_le_bytes(curr[685..693].try_into().unwrap());
    assert!(curr_fee < prev_fee, "fee_reserve must strictly decrease per hop -- every assembled heartbeat still costs something");
    assert!(curr_fee > 0, "organism starved mid-batch -- fee_reserve hit zero before the batch finished");

    let expected_prev_hash = sha256(prev);
    assert_eq!(&curr[493..525], &expected_prev_hash, "hop memory chain broken -- each assembled hop must link to the exact prior hop");

    // Every other byte must be frozen: mask out the fields we just verified as mutable
    // (generation, prev_state_hash, transition_count, fee_reserve), plus the metabolism
    // rate-window bookkeeping (window_start_daa, window_spend_count) -- those are only
    // meaningful once, for the single on-chain footprint this whole batch produces (that
    // accounting is enforced separately, on the OUTER old_dna/new_dna pair, by the
    // existing Phase 1c logic -- unmodified, exactly as for any ordinary heartbeat).
    let mut p = prev.to_vec();
    let mut c = curr.to_vec();
    for r in [36..40usize, 41..42usize, 493..525usize, 525..529usize, 685..693usize, 709..721usize, 902..914usize] {
        for i in r {
            p[i] = 0;
            c[i] = 0;
        }
    }
    assert_eq!(p, c, "only generation/prev_state_hash/transition_count/fee_reserve/rate-window/ARMA bookkeeping may change within a batch hop -- every other chromosome is frozen mid-assembly");
}
...
fn main() {
    ...
    let transition: u8 = env::read(); // 0=owner-sig, 1=XMSS check-in, 2=privacy withdrawal, 3=game resolve, 4=route moult, 5=reproduction (spawn children), 15=world genesis (spawn a world), 16=ARMA batch consent, 17=atomic swap (HTLC init/claim/refund)

    // ── Chromosome 19 (ARMA): batch_size is read ONLY for transition 16. Every other
    // transition kind implicitly has batch_size = 1 -- a single ordinary hop, exactly the
    // behavior every prior stage already assumed. This lets one shared code path (below)
    // serve both "one heartbeat" and "an assembler's whole batch of heartbeats" without
    // duplicating the generation/transition_count logic.
    let batch_size: u32 = if transition == 16 { env::read() } else { 1 };
    if transition == 16 {
        assert!(batch_size >= 2, "an ARMA batch must actually batch more than one hop -- otherwise just use an ordinary transition");
    }
    ...
    assert_eq!(new_dna.generation, old_dna.generation + batch_size, "generation must advance by exactly batch_size (1 for every ordinary transition)");
    ...
    if transition == 16 {
        // ── Chromosome 19 (ARMA) exception ──  new_dna's real predecessor is the LAST
        // internal hop assembled off-chain, not old_dna directly (unless batch_size==1,
        // which ARMA never uses -- see the batch_size>=2 check above). The transition==16
        // branch below walks the full internal chain and verifies, hop by hop, that it
        // starts at old_dna and ends at new_dna -- so the correct link is checked there,
        // transitively, instead of directly here.
        assert_eq!(new_dna.transition_count, old_dna.transition_count + batch_size, "transition_count must advance by exactly batch_size");
        ...
        assert_eq!(new_dna.transition_count, old_dna.transition_count + batch_size, "parent's transition_count must advance by exactly batch_size");
    ...
    // ── Phase 1e: newborn incubation invariant (Chromosome 17 -- "111", the angel number) ──
    // A phoenix that has just risen from absolute dust is not yet strong enough to leave
    // the cradle. It must rest for exactly 111 DAA -- the number of alignment, of new
    // beginnings made literal -- before its first true step out of STEM. This is a second,
    // shorter mirror of Stage 16's three-day tomb: that one gates the RISING itself, this
    // one gates the first departure AFTER rising. Once earned, it is never re-checked --
    // reborn_at_daa is a permanent historical marker, not a recurring deadline.
    if transition == 14 {
        assert_eq!(new_dna.reborn_at_daa, current_daa, "ignition must record the exact moment of rebirth");
    } else {
        assert_eq!(new_dna.reborn_at_daa, old_dna.reborn_at_daa, "reborn_at_daa is historical record -- only IGNITE can set it");
        if old_dna.mode == MODE_STEM && old_dna.reborn_at_daa != 0 && new_dna.mode != MODE_STEM {
            assert!(current_daa >= old_dna.reborn_at_daa + INCUBATION_DAA,
                "the newborn must rest 111 DAA in the cradle before its first true step out of stem");
        }
    }

    // ── Phase 1f: world genesis invariant (Chromosome 18 -- "a scorpion who generates
    // worlds") ──  world_chain_hash and worlds_spawned are append-only, extended by
    // exactly one transition kind. max_worlds is a birth-time constant, never mutable,
    // not even by the one transition that is allowed to touch the other two fields.
    if transition != 15 {
        assert_eq!(new_dna.world_chain_hash, old_dna.world_chain_hash, "world_chain_hash is append-only -- only world genesis can extend it");
        assert_eq!(new_dna.worlds_spawned, old_dna.worlds_spawned, "worlds_spawned unchanged outside world genesis");
        assert_eq!(new_dna.max_worlds, old_dna.max_worlds, "max_worlds is set at birth, immutable");
    }

    // ── Phase 1g: ARMA invariant (Chromosome 19 -- assembly/consent separation) ──
    // last_batch_size and total_batched_transitions are touched by exactly one transition
    // kind. total_batched_transitions is a permanent, lifetime, never-resets counter --
    // even through death and rebirth -- proof of how much this organism has ever processed
    // via horizontal batching, the same "eternal memory" ethos as world_chain_hash.
    if transition != 16 {
        assert_eq!(new_dna.last_batch_size, old_dna.last_batch_size, "last_batch_size unchanged outside ARMA consent");
        assert_eq!(new_dna.total_batched_transitions, old_dna.total_batched_transitions, "total_batched_transitions unchanged outside ARMA consent");
    } else {
        assert_eq!(new_dna.last_batch_size, batch_size, "last_batch_size must record the exact batch_size just consented to");
        assert_eq!(
            new_dna.total_batched_transitions,
            old_dna.total_batched_transitions + batch_size as u64,
            "total_batched_transitions must advance by exactly batch_size"
        );
    }

    // ── Phase 1h: crosschain atomic transfer invariant (Chromosome 20 -- HTLC) ──
    // swap_hash / swap_timeout_daa / swap_active are touched by exactly one transition
    // kind. Outside it, they must be frozen -- an unrelated heartbeat or moult can never
    // silently cancel or tamper with a pending cross-chain swap.
    if transition != 17 {
        assert_eq!(new_dna.swap_hash, old_dna.swap_hash, "swap_hash unchanged outside atomic swap");
        assert_eq!(new_dna.swap_timeout_daa, old_dna.swap_timeout_daa, "swap_timeout_daa unchanged outside atomic swap");
        assert_eq!(new_dna.swap_active, old_dna.swap_active, "swap_active unchanged outside atomic swap");
    }

    // Chromosome 20: set only when transition==17, action==CLAIM -- the revealed HTLC
    // preimage is committed to the public journal below so any cross-chain observer can
    // extract it and complete their own leg of the swap. This is the entire mechanism
    // that makes it a real atomic swap instead of just a local timelock.
    let mut revealed_secret: Vec<u8> = Vec::new();
    ...
    if transition == 0 {
        ...
        // reborn_at_daa is set to current_daa -- already enforced globally in Phase 1e above.
    } else if transition == 15 {
        // ── Stage 18: WORLD GENESIS (Chromosome 18) -- a scorpion who generates worlds ──
        // Reproduction (transition 5) spawns more scorpions: same DNA shape, same rules,
        // a new instance of the same species. This is categorically different -- the
        // scorpion authorizes a brand-new covenant genesis output that does not have to
        // be a scorpion at all. Any redeem-script template, any rule set, any species:
        // a zktimelock, a sentinel, a zkgate, another organism entirely, or something
        // that has never existed before. The scorpion doesn't reproduce itself here --
        // it seeds an entire independent world and lets it go free.
        //
        // Provenance stays fully verifiable: every world this scorpion has ever seeded,
        // across its entire existence -- even through death and rebirth (Chromosome
        // 14/17) -- is permanently linked into one forward hash chain, exactly like
        // Chromosome 7's memory chain, but for children of a different kind.
        assert_eq!(old_dna.mode, MODE_VAULT, "only a living, mature scorpion (vault) can generate a world");
        assert_eq!(new_dna.mode, MODE_VAULT, "generating a world does not change the scorpion's own mode");

        assert_eq!(new_dna.max_worlds, old_dna.max_worlds, "max_worlds is set at birth, immutable");
        assert_eq!(
            new_dna.worlds_spawned,
            old_dna.worlds_spawned + 1,
            "worlds_spawned must advance by exactly 1"
        );
        assert!(
            new_dna.worlds_spawned <= old_dna.max_worlds,
            "cannot exceed max_worlds -- even a god of creation has a limit"
        );

        let world_covenant_id_v: Vec<u8> = env::read();
        assert_eq!(world_covenant_id_v.len(), 32, "world_covenant_id must be 32 bytes");
        let world_covenant_id: [u8; 32] = world_covenant_id_v.try_into().unwrap();

        let world_script_hash_v: Vec<u8> = env::read();
        assert_eq!(world_script_hash_v.len(), 32, "world_script_hash must be 32 bytes");
        let world_script_hash: [u8; 32] = world_script_hash_v.try_into().unwrap();

        let mut buf = Vec::with_capacity(96);
        buf.extend_from_slice(&old_dna.world_chain_hash);
        buf.extend_from_slice(&world_covenant_id);
        buf.extend_from_slice(&world_script_hash);
        let expected_chain_hash = sha256(&buf);
        assert_eq!(
            new_dna.world_chain_hash, expected_chain_hash,
            "world_chain_hash must extend the chain with exactly this new world's identity"
        );
    } else if transition == 16 {
        // ── Stage 19: ARMA -- assembly/consent separation (Chromosome 19) ──
        // The real Arma design splits a BFT pipeline into many parallel ASSEMBLERS
        // (who batch and chain data, scaling horizontally with bandwidth/CPU) and one
        // thin CONSENTER (who only orders/finalizes batch commitments). Here: an
        // off-chain assembler pre-computed an entire internal chain of `batch_size`
        // ordinary heartbeats. This single proof is the consenter -- it walks and
        // verifies every internal hop, then finalizes only the LAST state on-chain.
        // The organism's own throughput scales with what its assembler prepared, while
        // the on-chain footprint stays exactly one transition, same as every other kind.
        assert_eq!(old_dna.mode, MODE_VAULT, "an ARMA batch can only be assembled from a living, mature scorpion (vault)");
        assert_eq!(new_dna.mode, MODE_VAULT, "an ARMA batch cannot change the scorpion's own mode -- it's a batch of ordinary heartbeats");

        let inner_hops: Vec<Vec<u8>> = env::read();
        assert_eq!(
            inner_hops.len() as u32, batch_size - 1,
            "inner_hops must contain exactly batch_size - 1 intermediate states (old_dna and new_dna are the two ends)"
        );

        // Walk the full internal chain: old_dna -> inner_hops[0] -> ... -> inner_hops[n-1] -> new_dna.
        let mut prev_bytes: &[u8] = &old_dna_bytes;
        for hop_bytes in inner_hops.iter() {
            assert_hop_valid(prev_bytes, hop_bytes);
            prev_bytes = hop_bytes;
        }
        assert_hop_valid(prev_bytes, &new_dna_bytes);

        // last_batch_size / total_batched_transitions are already checked globally in Phase 1g.
    } else if transition == 17 {
        // ── Stage 20: CROSSCHAIN ATOMIC TRANSFER (Chromosome 20) -- HTLC ──
        // The classic hash-and-time-locked contract that makes atomic swaps between
        // independent chains possible: commit to a hash, and either (a) whoever reveals
        // the matching preimage before the deadline unlocks it -- proving they completed
        // their leg of the swap on the OTHER chain, using the SAME secret -- or (b) if
        // nobody claims in time, the original owner gets it back, permissionlessly, no
        // signature required, exactly like Chromosome 3's sentinel dead-man's-switch.
        //
        // action: 0=INIT (commit to a new swap), 1=CLAIM (reveal the preimage, unlock),
        // 2=REFUND (timeout passed, nobody claimed, permissionless cancel).
        assert_eq!(old_dna.mode, MODE_VAULT, "only a living, mature scorpion (vault) can enter a cross-chain swap");
        assert_eq!(new_dna.mode, MODE_VAULT, "an atomic swap action does not change the scorpion's own mode");

        let action: u8 = env::read();
        if action == 0 {
            // ── INIT: commit to a fresh swap. Cannot overlap an already-pending one. ──
            assert_eq!(old_dna.swap_active, 0, "cannot INIT a new swap while one is still pending -- resolve it first (CLAIM or REFUND)");

            let new_swap_hash_v: Vec<u8> = env::read();
            assert_eq!(new_swap_hash_v.len(), 32, "swap_hash must be 32 bytes");
            let new_swap_hash: [u8; 32] = new_swap_hash_v.try_into().unwrap();
            let timeout_daa: u64 = env::read();

            assert!(timeout_daa >= current_daa + MIN_SWAP_WINDOW_DAA,
                "swap timeout must be at least MIN_SWAP_WINDOW_DAA in the future -- an instant or near-instant deadline gives the counterparty no real window to complete their leg");

            assert_eq!(new_dna.swap_hash, new_swap_hash, "swap_hash must be set to exactly the committed hash");
            assert_eq!(new_dna.swap_timeout_daa, timeout_daa, "swap_timeout_daa must be set to exactly the chosen deadline");
            assert_eq!(new_dna.swap_active, 1, "swap_active must be set on INIT");
        } else if action == 1 {
            // ── CLAIM: reveal the preimage. Anyone who knows the secret can unlock -- ──
            // this is deliberately NOT owner-gated, exactly like a real HTLC: knowledge
            // of the secret IS the authorization, because that secret is the same one
            // the counterparty revealed on the other chain to let US claim over there.
            assert_eq!(old_dna.swap_active, 1, "cannot CLAIM -- no swap is currently pending");

            let secret: Vec<u8> = env::read();
            let computed_hash = sha256(&secret);
            assert_eq!(computed_hash, old_dna.swap_hash, "revealed secret does not match the committed swap_hash");

            assert_eq!(new_dna.swap_hash, [0u8; 32], "swap_hash must be cleared on successful claim");
            assert_eq!(new_dna.swap_timeout_daa, 0, "swap_timeout_daa must be cleared on successful claim");
            assert_eq!(new_dna.swap_active, 0, "swap_active must be cleared on successful claim");

            revealed_secret = secret;
        } else if action == 2 {
            // ── REFUND: timeout passed, nobody claimed -- permissionless, no signature. ──
            assert_eq!(old_dna.swap_active, 1, "cannot REFUND -- no swap is currently pending");
            assert!(current_daa >= old_dna.swap_timeout_daa, "cannot REFUND before the swap's timeout has actually passed");

            assert_eq!(new_dna.swap_hash, [0u8; 32], "swap_hash must be cleared on refund");
            assert_eq!(new_dna.swap_timeout_daa, 0, "swap_timeout_daa must be cleared on refund");
            assert_eq!(new_dna.swap_active, 0, "swap_active must be cleared on refund");
        } else {
            panic!("unknown atomic swap action");
        }
    } else {
    ...
    let mut journal = Vec::with_capacity(32 + 32 + 1 + 1 + revealed_secret.len());
    ...
    // Chromosome 20: if this was a successful CLAIM, the preimage is committed here in
    // the clear -- publicly, permanently, on the receipt's journal -- so any observer on
    // the OTHER chain of the swap can read it straight off this proof and complete their
    // own leg with the exact same secret. That public reveal is the entire mechanism.
    journal.extend_from_slice(&revealed_secret);
