...
function arg2(name, def) { const a = process.argv.find(x => x.startsWith(`--${name}=`)); return a ? a.split('=').slice(1).join('=') : def; }
const PUBKIT_PATH = arg2('pubkit', null);
const REDEEM_SCRIPT_HEX = PUBKIT_PATH
  ? JSON.parse(fs.readFileSync(PUBKIT_PATH, 'utf8')).redeem_script_hex
  : fs.readFileSync('./xmss_generic_deploy_script.hex', 'utf8').trim();
