...
function scriptNum(n) {
  ...
  const neg = n < 0; let abs = Math.abs(n); const bytes = [];
...
function pushData(buf) {
  ...
  const lb = Buffer.alloc(2); lb.writeUInt16LE(buf.length);
  return Buffer.concat([Buffer.from([0x4d]), lb, buf]);
}
const BLOCKS = parseInt((process.argv.find(x=>x.startsWith("--blocks="))||"--blocks=15").split("=")[1]);
const XMSS_SCRIPT_HEX = fs.readFileSync("./time_capsule_xmss_script.hex", "utf8").trim();
...
async function main() {
  ...
  const UNLOCK_DAA = currentDaa + BLOCKS;
  const OP_CLTV = 0xb0;
  const xmssScript = Buffer.from(XMSS_SCRIPT_HEX, 'hex');
  const redeemScript = Buffer.concat([pushData(scriptNum(UNLOCK_DAA)), Buffer.from([OP_CLTV]), xmssScript]);
  console.log('Unlock DAA:', UNLOCK_DAA, '(current', currentDaa, ')');
